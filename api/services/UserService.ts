import { User } from "../../interfaces/User";
import { supabase } from "../supabase";
import { compareHashed } from "../../utils/compareHashed";
import { hashPassword } from "../../utils/hashPassword";
import { Provider } from "../../interfaces/Provider";
import { Budget } from "../../interfaces/Budget";
import { MONTHLY_BUDGETS, USERS } from "../../constants/Tables";
import { CONVERT_BUDGETS_CURRENCY, GET_USER_BUDGETS } from "../../constants/PostgresFunctions";
import { CategoryService } from "./CategoryService";
import { WalletService } from "./WalletService";

const registerUser = async (user: User) => {
  const { firstName, lastName, email, password, provider } = user;

  try {
    const { data } = await supabase
      .from(USERS)
      .select("*")
      .filter("email", "eq", email)
      .filter("provider", "eq", provider)
      .single();

    if (data) {
      return {
        title: "Try again",
        message: "This user is already registered !",
      };
    } else {
      const { error } = await supabase.from(USERS).insert(
        [
          {
            first_name: firstName,
            last_name: lastName,
            email,
            password: await hashPassword(password),
            provider,
          },
        ],
        {
          returning: "minimal",
        }
      );
      if (error) {
        throw error;
      }

      // Seed the new account with the default set of categories.
      try {
        const { data: created } = await supabase
          .from(USERS)
          .select("id")
          .filter("email", "eq", email)
          .filter("provider", "eq", provider)
          .single();

        if (created?.id) {
          await CategoryService.seedDefaultCategories(created.id);
          await WalletService.seedDefaultWallet(created.id);
        }
      } catch (seedError) {
        console.log("seedDefaultCategories failed:", seedError);
      }

      return {
        title: "Success",
        message: "User succesfully created !",
      };
    }
  } catch (error) {
    if (error instanceof Error) {
      console.log(error);
    }
  }
};

const resetPassword = async (email: string, newPassword: string) => {
  try {
    const { data } = await supabase.from(USERS).select("*").filter("email", "eq", email).single();

    if (!data) {
      return {
        message: "This user does not exist !",
      };
    } else {
      await supabase
        .from(USERS)
        .update({ password: await hashPassword(newPassword) })
        .filter("email", "eq", email);

      return {
        message: "Password resetted succesfully !",
      };
    }
  } catch (error) {
    if (error instanceof Error) {
      console.log(error);
    }
  }
};

const changePassword = async (email: string, newPassword: string) => {
  const hashedPassword = await hashPassword(newPassword);

  try {
    const { error } = await supabase
      .from(USERS)
      .update({ password: hashedPassword })
      .filter("email", "eq", email);

    if (error) {
      throw error;
    }

    return {
      message: "Password changed succesfully !",
    };
  } catch (error) {
    if (error instanceof Error) {
      console.log(error);
    }
  }
};

const loginUser = async (email: string, password: string, provider: Provider) => {
  try {
    const { data } = await supabase
      .from(USERS)
      .select("*")
      .filter("email", "eq", email)
      .filter("provider", "eq", provider)
      .single();

    if (data && (await compareHashed(password, data.password))) {
      return {
        message: "User exists",
        data: data,
      };
    } else {
      return {
        message: "Invalid email or password !",
      };
    }
  } catch (error) {
    return {
      message: error,
    };
  }
};

const getUserBudgets = async (userId: number, walletId: number) => {
  try {
    const { data, error } = await supabase.rpc(GET_USER_BUDGETS, {
      user_id: userId,
      wallet_id: walletId,
    });

    if (error) {
      throw error;
    }

    return Array.isArray(data) ? data : [];
  } catch (error) {
    console.log("getUserBudgets failed:", error);
    return [];
  }
};

const saveUserBudgets = async (userId: number, walletId: number, budgets: Budget[]) => {
  try {
    for (const item of budgets) {
      const { data } = await supabase
        .from(MONTHLY_BUDGETS)
        .select("*")
        .filter("category_id", "eq", item.id)
        .filter("user_id", "eq", userId)
        .filter("wallet_id", "eq", walletId)
        .single();

      if (data) {
        await supabase
          .from(MONTHLY_BUDGETS)
          .update({ budget: item.budget })
          .filter("category_id", "eq", item.id)
          .filter("user_id", "eq", userId)
          .filter("wallet_id", "eq", walletId);
      } else {
        await supabase.from(MONTHLY_BUDGETS).insert({
          user_id: userId,
          category_id: item.id,
          budget: item.budget,
          wallet_id: walletId,
        });
      }
    }
  } catch (error) {
    if (error instanceof Error) {
      console.log(error);
    }
  }
};

const convertUserBudgetsCurrency = async (userId: number, conversionRate: number) => {
  try {
    const { error } = await supabase.rpc(CONVERT_BUDGETS_CURRENCY, {
      user_id: userId,
      conversion_rate: conversionRate,
    });

    if (error) {
      throw error;
    }
  } catch (error) {
    if (error instanceof Error) {
      console.log(error);
    }
  }
};

const removeUserBudgets = async (userId: number) => {
  try {
    const { error } = await supabase.from(MONTHLY_BUDGETS).delete().filter("user_id", "eq", userId);

    if (error) {
      throw error;
    }
  } catch (error) {
    if (error instanceof Error) {
      console.log(error);
    }
  }
};

const verifyPassword = async (email: string, password: string): Promise<boolean> => {
  try {
    const { data, error } = await supabase
      .from(USERS)
      .select("password")
      .filter("email", "eq", email)
      .single();
    if (error || !data?.password) return false;
    return await compareHashed(password, data.password);
  } catch {
    return false;
  }
};

export const UserService = {
  registerUser,
  loginUser,
  getUserBudgets,
  saveUserBudgets,
  convertUserBudgetsCurrency,
  resetPassword,
  changePassword,
  removeUserBudgets,
  verifyPassword,
};
